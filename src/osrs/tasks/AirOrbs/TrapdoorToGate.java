...
public class TrapdoorToGate extends Task {

    public final int TRAPDOOR_CLOSED=1579;
    public final int TRAPDOOR=1581;
    public final int GATE_ONE_CLOSED[]=new int[]{1568,1569};
    public final int GATE_ONE[]=new int[]{1571,1572};
    ...
    public static final Tile[] LADDER_TO_DOOR = {new Tile(3096, 9867, 0), new Tile(3096, 9871, 0), new Tile(3096, 9875, 0), new Tile(3096, 9879, 0), new Tile(3096, 9883, 0), new Tile(3095, 9887, 0), new Tile(3095, 9891, 0), new Tile(3095, 9895, 0), new Tile(3095, 9899, 0), new Tile(3096, 9903, 0), new Tile(3100, 9905, 0), new Tile(3103, 9909, 0)};
    ...
    public static final Tile[] myPath = LADDER_TO_DOOR;

    public Walker walker  = new Walker(ctx);
    public Tile doorTile= myPath[myPath.length-1];



    public TrapdoorToGate(ClientContext ctx) {
        super(ctx);
    }

    @Override
    public boolean activate() {
        for(Tile tile: LADDER_TO_DOOR)
    ...
    @Override
    public void execute() {
        if(ctx.objects.select().id(GATE_ONE_CLOSED).nearest().poll().inViewport()){
            ctx.objects.select().id(1509).nearest().poll().interact("Open");

        }
        else if(ctx.objects.select().id(GATE_ONE).nearest().poll().inViewport()){
//            ctx.objects.select().id(GATE_ONE).nearest().poll().click();
        }
