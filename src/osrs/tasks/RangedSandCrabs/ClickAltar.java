...
public class ClickAltar extends Task
{
    ...
    public ClickAltar(ClientContext ctx) {
        super(ctx);
    }

    @Override
    public boolean activate() {
        return ctx.prayer.prayerPoints()<60 && ctx.objects.id(ALTAR).nearest().poll().inViewport();
    }

    @Override
    public void execute() {
        ctx.objects.id(ALTAR).nearest().poll().interact("Pray");
